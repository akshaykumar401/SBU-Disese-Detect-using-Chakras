...
def upload():
  ...
  if file:
    ...
    file.save(f'netlify/uploads/{filename}')
    
    # Converting the Audio to mp3
    subprocess.run(['ffmpeg', '-i', f'netlify/uploads/{filename}', f'netlify/uploads/mp3/{filename}.mp3'])
    
    # Extracting features
    features = extract_features(file.filename)
  
    # After Extracting Deleting the audio file from server
    os.remove(f'netlify/uploads/{filename}')
    os.remove(f'netlify/uploads/mp3/{filename}.mp3')
